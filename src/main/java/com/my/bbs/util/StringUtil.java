...
public class StringUtil {
	...
	public static final boolean isNumberDecimal(String str) {
	...
	public static final String getRandomZMString(int length) {
		if (length < 1) {
			return "";
		} else {
			Random random = new Random();
			int str_len = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".length();
			StringBuilder b = new StringBuilder();

			for (int i = 0; i < length; ++i) {
				b.append("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".charAt(random.nextInt(str_len)));
			}
	...
	public static String supplementZero(int numLength, int randomNum) {
		 ...
		 * d 代表为正数。
		 */
		String newString = String.format("%0" + numLength + "d", randomNum);
		return newString;
	}


	public static void main(String[] args) {
		System.out.println(StringUtil.isNumberDecimal("2.5a"));
		System.out.println(StringUtil.isNumberDecimal("2a"));
	}
